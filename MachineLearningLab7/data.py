...
def visualize_tree(node, dot=None):
    if dot is None:
        dot = Digraph()
        dot.node(name=str(id(node)), label=str(node.feature_name))

    for value, child_node in node.children.items():
        if child_node is not None:
            dot.node(name=str(id(child_node)), label=str(child_node.feature_name if not child_node.leaf else child_node.prediction))
    ...
    for max_depth in max_depths:
        for min_samples_split in min_samples_splits:
            for min_samples_leaf in min_samples_leafs:
                ...
                #将decision_tree可视化 并绘制树模型图
                dot = visualize_tree(decision_tree)
                # 保存为svg   
                dot.render('decision_tree.gv', view=True)  # 保存并打开生成的图像
